    ...
    def getCardList(self): # appends all of the cards with their: value, suit and file name
        ...
        for number in range(13): # loops through the cardList construction
            if number == 0: # for the first item (ace) the value is the highest but the number on the card is lowest
                self.cardList.append([self.aceValue, "C", "Assets/C"+str(number+1)+".JPG"]) # Changed order from F, S, V to V, S, F
            else: # adds the rest of the suit (2-K)to the cardList
                self.cardList.append([number, "C" , "Assets/C"+str(number+1)+".JPG"])
        for number in range(13): # runs 4 times to add each suit
            if number == 0:
                self.cardList.append([self.aceValue, "D", "Assets/D"+str(number+1)+".JPG"])
            else:
                self.cardList.append([number, "D", "Assets/D"+str(number+1)+".JPG"])
        for number in range(13):
            if number == 0:
                self.cardList.append([self.aceValue, "H", "Assets/H"+str(number+1)+".JPG"])
            else:
                self.cardList.append([number, "H", "Assets/H"+str(number+1)+".JPG"])
        for number in range(13):
            if number == 0:
                self.cardList.append([self.aceValue, "S", "Assets/S"+str(number+1)+".JPG"])
            else:
                self.cardList.append([number, "S", "Assets/S"+str(number+1)+".JPG"])
...
class User(Player):
    def __init__(self, locPos):
        super().__init__()
        self.locPos = locPos
        self.betButtonX1 = 723
        self.betButtonX2 = 823
        self.betButtonY1 = 600
        self.betButtonY2 = 650
        self.betButtonTextX = 726
        self.betButtonTextY = 610
        self.callButtonY1 = 660
        self.callButtonX1 = 823
        self.callButtonY2 = 710
        self.callButtonTextY = 670
        self.foldButtonX1 = 833
        self.foldButtonX2 = 933
        self.foldButtonTextX = 838
        self.increaseBetX1 = 950
        self.increaseBetX2 = 1000
        self.betCoverX = 1015
        self.betCoverY = 635
        self.betCoverWidth = 80
        self.betCoverHeight = 40
        self.chipsCoverX = 445
        self.chipsCoverY = 640
        self.chipsCoverWidth = 105
        self.betChange = 50
        self.increaseTextBoxX = 1020
        self.chipsTextX = 450
        self.addBetX = 1095
        self.computerBetBoxPos = (557.5, 485)
        self.betTextPos = (562.5, 490)
            ...
            while True:
                for event in pygame.event.get():
                    ...
                    if event.type == pygame.MOUSEBUTTONDOWN: # if a click is detected
                        if pygame.mouse.get_pos()[0] >= self.betButtonX1 and pygame.mouse.get_pos()[1] >= self.betButtonY1: # if the mouse x is greater than 723 and the y is greater than 600
                            if pygame.mouse.get_pos()[0] <= self.betButtonX2 and pygame.mouse.get_pos()[1] <= self.betButtonY2: # if the mouse x is less than 823 and y is less than 650
                                screen.blit(assets.gamePress, (self.betButtonX1, self.betButtonY1)) # display the pressed button
                                screen.blit(assets.betButtonText, (self.betButtonTextX, self.betButtonTextY)) # display the text over the top of that
                                ...
                                screen.blit(assets.userButton, (self.betButtonX1, self.betButtonY1)) # returns the buttons to its original look
                                screen.blit(assets.betButtonText, (self.betButtonTextX, self.betButtonTextY)) # ^^
                        ...
                        if pygame.mouse.get_pos()[0] >= self.foldButtonX1 and pygame.mouse.get_pos()[1] >= self.betButtonY1: # Fold button
                            if pygame.mouse.get_pos()[0] <= self.foldButtonX2 and pygame.mouse.get_pos()[1] <= self.betButtonY2:
                                ...
                                pygame.display.update()
                                time.sleep(0.2)
                                screen.blit(assets.userButton, (self.foldButtonX1, self.betButtonY1))
                                screen.blit(assets.foldButtonText, (self.foldButtonTextX, self.betButtonTextY))
                                pygame.display.update()
                                self.fold()
                                game.previousMove = "Fold"
                                return # this will allow the user to get out of the round if they think that they cannot win
        ...
        while self.betting:
            for event in pygame.event.get():
                if event.type == pygame.MOUSEBUTTONDOWN:
                    if pygame.mouse.get_pos()[0] >= self.increaseBetX1 and pygame.mouse.get_pos()[1] >= self.betButtonY1:  # Increase bet button
                        if pygame.mouse.get_pos()[0] <= self.increaseBetX2 and pygame.mouse.get_pos()[1] <= self.betButtonY2:
                            if user.Chips > 0: # checks that the user has chips to bet
                                pygame.draw.rect(screen, Colours._lightGrey, (self.betCoverX, self.betCoverY, self.betCoverWidth, self.betCoverHeight), 0)  # Rect over the top of the increase to cover old text
                                pygame.draw.rect(screen, Colours._lightGrey, (self.chipsCoverX, self.chipsCoverY, self.chipsCoverWidth, self.betCoverHeight), 0)  # Rect over users chips to update the text
                                self.increaseBet = self.increaseBet + self.betChange # increases the bet by 50 as this is the increment cause by the button press
                                self.Chips = self.Chips - self.betChange # takes these chips out of the users bank as they would not gave them
                                ...
                                screen.blit(self.userIncreaseText, (self.increaseTextBoxX, self.chipsCoverY))  # Displays the new increase over the top
                                screen.blit(self.userChipsText, (self.chipsTextX, self.chipsCoverY))  # Displays the new user chips
                                screen.blit(assets.upBetPress, (self.increaseBetX1, self.betButtonY1))
                                pygame.display.update()
                                time.sleep(0.12)
                                screen.blit(assets.upBet, (self.increaseBetX1, self.betButtonY1))  # Back to grey button
                                pygame.display.update()
                    ...
                    if self.increaseBet > game.previousBet:
                        if pygame.mouse.get_pos()[0] >= self.increaseBetX1 and pygame.mouse.get_pos()[1] >= self.callButtonY1:  # Decrease bet button
                            if pygame.mouse.get_pos()[0] <= self.increaseBetX2 and pygame.mouse.get_pos()[1] <= self.callButtonY2:
                                if self.increaseBet > 0:
                                    ...
                                    pygame.draw.rect(screen, Colours._lightGrey, (self.chipsCoverX, self.chipsCoverY, self.chipsCoverWidth, self.betCoverHeight), 0)
                                    self.increaseBet = self.increaseBet - self.betChange # same as the previous section but reversed
                                    user.Chips = user.Chips + self.betChange # adds the chips taken from the increase and returned to bank
                                    ...
                                    screen.blit(self.userIncreaseText, (self.increaseTextBoxX, self.chipsCoverY))
                                    screen.blit(self.userChipsText, (self.chipsTextX, self.chipsCoverY))
                                    screen.blit(assets.downBetPress, (self.increaseBetX1, self.callButtonY1))
                                    pygame.display.update()
                                    time.sleep(0.12)
                                    screen.blit(assets.downBet, (self.increaseBetX1, self.callButtonY1))
                                    pygame.display.update()

                    if pygame.mouse.get_pos()[0] >= self.betCoverX and pygame.mouse.get_pos()[1] >= self.betCoverY:  # Add increase to total bet
                        if pygame.mouse.get_pos()[0] <= self.addBetX and pygame.mouse.get_pos()[1] <= self.callButtonTextY:
...
class Computer(Player):
    ...
    def nextTurn(self, stage): # the next turn for the computer is determined
        ...
        if self.isFolded == True or self.Chips == 0:
            ...
            if game.stage == 4:
                ...
                self.currentInfoList = ["Root", self.potentialValue, str(self.potentialValue)+game.previousMove]
                self.computerSelection(self.potentialGraph, "Root")

            if len(self.moves) == 4:
                getattr(self, self.moves[-1])()
        ...
        while queue:
            ...
            if currentNode in self.currentInfoList:
                ...
                for neigbour in neighbours:
                    ...
                    if len(self.moves) == 3 and neighbours == "call" or neighbours == "fold" or neighbours == "computerBet":
                        self.moves.append(neighbours)
                # except:
                #     # print(currentNode)
        return self.moves
    ...
    def fold(self):
        ...
        self.isFolded = True
        self.foldThisTurn = True
        game.blankCards(game.remainingPlayers[self.locPos][4])
        game.updateUserChips()
        game.updatePot()
        #del game.remainingPlayers[self.locPos]
        #game.remainingList.remove(self.locPos)
        game.remainingPlayers[self.locPos][0] = 10
        game.remainingList[self.locPos] = 10
        game.previousMove = "Fold"
...
class Game():
    def __init__(self):
        self.compCardY = 10
        self.backLoc = (0, 0)
        self.tableLoc = (150, 150)
        self.backButLoc = (30, 620)
        self.backButTextLoc = (65, 625)
        self.cardWidth = 75
        self.potBoxLoc = (557.5, 246)
        self.potTextLoc = (562.5, 251)
        self.comp1BetBoxLoc = (310.5, 178)
        self.comp2BetBoxLoc = (557, 178)
        self.comp3BetBoxLoc = (803.5, 178)
        self.chipsBackY = 123
        self.comp1ChipsTextLoc = (334, 119)
        self.comp2ChipsTextLoc = (581, 119)
        self.comp3ChipsTextLoc = (824, 119)
        self.comp1BetTextLoc = (315.5, 183)
        self.comp2BetTextLoc = (562, 183)
        self.comp3BetTextLoc = (808.5, 183)
        self.userChipsRectLoc = (445, 600, 105, 80)
        self.userBetBoxLoc = (557.5, 485)
        self.userBetTextLoc = (562.5, 490)
        self.loseLoc = (100, 100)
        self.flopY = 306
        self.flopDiff = 125
    ...
    def blankCards(self, playerToUpdate):
        if playerToUpdate == "user" or playerToUpdate == "all":
            for x in range(2):
                screen.blit(assets.blankCard, (self.playerCardsX, user.betButtonY1))
                self.playerCardsX = self.playerCardsX + self.cardWidth

        self.playerCardsX = 567

        if playerToUpdate == "computer2" or playerToUpdate == "all":
            for x in range(2):
                screen.blit(assets.blankCard, (self.playerCardsX, self.compCardY))
                self.playerCardsX = self.playerCardsX + self.cardWidth

        if playerToUpdate == "computer1" or playerToUpdate == "all":
            for x in range(2):
                screen.blit(assets.blankCard, (self.comp1X, self.compCardY))
                self.comp1X = self.comp1X + self.cardWidth

        if playerToUpdate == "computer3" or playerToUpdate == "all":
            for x in range(2):
                screen.blit(assets.blankCard, (self.comp3X, self.compCardY))
                self.comp3X = self.comp3X + self.cardWidth
    ...
    def startingAssets(self):
        ...
        screen.blit(assets.back, self.backLoc)
        screen.blit(assets.table, self.tableLoc)

        screen.blit(assets.userButton, (user.betButtonX1, user.betButtonY1)) # Bet button image
        screen.blit(assets.userButton, (user.betButtonX1, user.callButtonY1)) # Call button image
        screen.blit(assets.userButton, (user.foldButtonX1, user.betButtonY1)) # Fold button image
        screen.blit(assets.betButtonText, (user.betButtonTextX, user.betButtonTextY)) # Bet button text
        screen.blit(assets.callButtonText, (user.betButtonTextX, user.callButtonTextY)) # Call button text
        screen.blit(assets.foldButtonText, (user.foldButtonTextX, user.betButtonTextY)) # Fold button text
        screen.blit(assets.upBet, (user.increaseBetX1, user.betButtonY1))
        screen.blit(assets.downBet, (user.increaseBetX1, user.callButtonY1))
        self.blankCards(all)
        ...
        screen.blit(help.backPage, self.backButLoc) # the back button bottom left
        screen.blit(assets.backButtonText, self.backButTextLoc) # the text in ^^
    ...
    def displayCards(self, z):
        for each in self.remainingList:
            if each == 10:
                ...
                # # print("DC each", each)
                # # print("DC", self.remainingList)
                # # print("DC", self.remainingPlayers)
                ...
                if z == 1:
                    self.currentPlayerX = self.currentPlayerX + self.cardWidth
    ...
    def getDealerButtonLocation(self):
        ...
        self.dealerButtonLocation = 0 # random.randint(0, 3)
        ...
        while user.Chips > 0:
            ...
            for stage in self.stageList: # 4 is the number of stages (pre,  flop, turn, river)
                ...
                while self.betsEqual == False:
                    ...
                    if self.count > (4 - self.remainingList.count(10)):
                        self.isEqual()
    ...
    def updatePot(self):
        ...
        screen.blit(assets.computerBetBox, self.potBoxLoc)
        screen.blit(assets.potText, self.potTextLoc)
        pygame.display.update()
    ...
    def updateComputersChips(self): # refreshes all of the computer related chip assets
        ...
        screen.blit(assets.chipsBack, (self.playerCardsX, self.chipsBackY))
        screen.blit(assets.chipsBack, (self.comp3X, self.chipsBackY))
        screen.blit(assets.computer1ChipsText, self.comp1ChipsTextLoc)
        screen.blit(assets.computer2ChipsText, self.comp2ChipsTextLoc)
        screen.blit(assets.computer3ChipsText, self.comp3ChipsTextLoc)
        screen.blit(assets.computer1BetText, self.comp1BetTextLoc)
        screen.blit(assets.computer2BetText, self.comp2BetTextLoc)
        screen.blit(assets.computer3BetText, self.comp3BetTextLoc)
        pygame.display.update()

    def updateUserChips(self): # updates all of the user chip assets
        assets.generateNumber()
        pygame.draw.rect(screen, Colours._lightGrey, self.userChipsRectLoc, 0) # User total chips background (left)
    ...
    def getBlinds(self): # calls all functions relating to the blinds to start each round
        ...
        # print("Blind RP", self.remainingPlayers)
        # print("BLind RL", self.remainingList)
        # print("Blind tL", self.tempLocation)
    ...
    def constructRemainingPlayers(self):
        for x in range(4):
            if self.allPlayers[x][1].Chips > 0:
                self.remainingPlayers.update({x : [x, self.allPlayers[x][1], self.allPlayers[x][2], self.allPlayers[x][3], self.allPlayers[x][4]]})
            elif self.allPlayers[x][1].Chips <= 0:
                self.remainingPlayers.update({10 : [10, self.allPlayers[x][1], self.allPlayers[x][2], self.allPlayers[x][3], self.allPlayers[x][4]]})
    ...
    def restart(self):
        ...
        self.allPlayers = {0: [0, user, self.playerCardsX, 600, "user"], 1: [1, computer1, self.comp1X, 10, "computer1"], 2: [2, computer2, self.playerCardsX, 10, "computer2"], 3: [3, computer3, self.comp3X, 10, "computer3"]}
    ...
    def lose(self): # WIP
        screen.blit(assets.back, self.backLoc)
        screen.blit(assets.youLose, self.loseLoc)
        pygame.display.update()
    ...
    def displayFlop(self): # displays the first 3 cards (flop) from the river
        while self.flopX > 600:
            for i in range(3):
                ...
                screen.blit(self.flopCards, (self.flopX, self.flopY))
                self.flopX = self.flopX - self.flopDiff
                time.sleep(0.5)
                pygame.display.update()
    ...
    def displayTurn(self): # displays the fourth card in the river (turn)
        ...
        screen.blit(self.turnCard, self.turnLoc)
    ...
    def displayRiver(self): # displays the final card (river)
        ...
        screen.blit(self.riverCard, self.riverLoc)
...
class Help():
    def __init__(self):
        self.backScale = (1280, 720)
        self.nextButLoc = (175, 75)
        self.back = pygame.transform.scale(assets.back, self.backScale)
        ...
        self.nextPage = pygame.transform.scale(assets.menuButton, self.nextButLoc)
        self.backPage = self.nextPage
        self.nextPagePress = pygame.transform.scale(assets.menuButtonPress, self.nextButLoc)
                ...
                if event.type == QUIT:
                    ...
                    if pygame.mouse.get_pos()[0] >= 1075 and pygame.mouse.get_pos()[1] >= 620:
                        if pygame.mouse.get_pos()[0] <= 1250 and pygame.mouse.get_pos()[1] <= 695:
                            ...
                            screen.blit(assets.nextButtonText, (1110, user.betCoverY))
                            pygame.display.update()
                            time.sleep(0.1)
                            self.page2()
                    ...
                    if pygame.mouse.get_pos()[0] >= 30 and pygame.mouse.get_pos()[1] >= 620:
                        if pygame.mouse.get_pos()[0] <= 205 and pygame.mouse.get_pos()[1] <= 695:
                            ...
                            screen.blit(assets.backButtonText, (65, user.betCoverY))
                            pygame.display.update()
                            time.sleep(0.1)
                            menu.displayMenu()
    ...
    def page3(self):
        ...
        screen.blit(self.help17, (user.chipsCoverY, 80))
        screen.blit(self.help18, (10, 188))
        screen.blit(self.help19, (user.chipsCoverY, 188))
        screen.blit(self.help20, (10, 294))
        screen.blit(self.help21, (user.chipsCoverY, 294))
        screen.blit(self.help22, (10, 400))
        screen.blit(self.help23, (user.chipsCoverY, 400))
...
game = Game()
river = River()
locationList = {0: [0, user, 567, 600, "user"], 1: [1, computer1, 320, 10, "computer1"], 2: [2, computer2, 567, 10, "computer2"], 3: [3, computer3, 810, 10, "computer3"]}
help = Help()
menu = Menu()
