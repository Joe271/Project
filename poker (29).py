...
class Computer(Player):
    ...
    def nextTurn(self):
        if game.round <= 0:
            if self.currentBet < game.previousBet and game.previousBet<= self.maxBet:
                self.callandcheck()
    ...
    def callandcheck(self):
        print("Ubhio")
        self.bet = self.maxBet
        print("test",self.bet)

    def getMaxBets(self, playerPos):

        game.getHandValue(self.Cards, playerPos)

        if game.playerValue[playerPos] < 2:
            self.maxBet = 150
        elif game.playerValue[playerPos] < 4:
            self.maxBet = 1000
        elif game.playerValue[playerPos] < 6:
            self.maxBet = 2000
        elif game.playerValue[playerPos] < 8:
            self.maxBet = 3000
        elif game.playerValue[playerPos] < 10:
            self.maxBet = 5000

        print("computer max:", self.maxBet)
...
class Assets(): # All assets but cards
    def __init__(self):
        ...
        self.callButtonText = self.inGameFont.render("Check/Call", True, Colours.black)
...
class Game():
    ...
    def playGame(self):
        ...
        # Gets the final maximum bet that the computer will make after the river is displayed. Although this is got before the player can see the river it is not used before.
        computer.getMaxBets(1)
        computer2.getMaxBets(2)
        computer3.getMaxBets(3)
        ...
        print("Value list:", self.playerValue)

        while True:
            print("try", self.playerTurn)
            ...
            if user.bet == computer.bet and computer.bet == computer2.bet and computer2.bet == computer3.bet:
                if self.round <= 0:
                    self.displayFlop()
                elif self.round == 1:
                    self.displayTurn()
            ...
            if user.Chips >= 0:
                ...
                for event in pygame.event.get():
                    ...
                    if event.type == pygame.MOUSEBUTTONDOWN:
                        if pygame.mouse.get_pos()[0] >= 723 and pygame.mouse.get_pos()[1] >= 600: # Bet button
                        ...
                        if pygame.mouse.get_pos()[0] >= 30 and pygame.mouse.get_pos()[1] >= 620:       # Back button (return to menu)
